...
public class MasterClass 
{
	...
	private static String startplace, destination,dateOfSettingOff,balance;
	...
	public static void sendPass1(String Uname)
	{
		try {
			...
			int k = JOptionPane.showConfirmDialog(null, 
    				" Serious Problem! Please Run the Program again :( "
	...
	public static void readtrip(String userN) 
	{
		...
		savedTrip.getTripInfo(startplace,destination,dateOfSettingOff);
	...
	public static void readBalance(String userN) 
	{
		try{
		...
		name = userN;
		System.out.println(name);
		FileReader fstream = new FileReader(name+"'s Balance"+".txt");
		BufferedReader in = new BufferedReader(fstream);
		Scanner scan = new Scanner(in);
		...
		System.out.println("Balance is: " + list.get(0));
	

		System.out.print(s);	

	AccountHomePage.getBalanceFromFile(balance);
		...
		{
			JOptionPane.showMessageDialog(null," Please Deposit some more money in RPS Bank Account :) "
					, " ERROR! ",
					  JOptionPane.ERROR_MESSAGE);
		}
	}
